...
describe BumpEnvironments do
  ...
  context 'load_node_attr' do
    it 'Load node attributes' do
      ...
      expect(BumpEnvironments.default_environmnets_word).to match(/~/)
      expect(BumpEnvironments.all_environmnets_word).to match(/\*/)
      expect(BumpEnvironments.chef_repo).to match(/osuosl\/chef-repo/)
      expect(BumpEnvironments.chef_repo).to match(/github_token/)
    end
  end
